...
#   6. Tears everything down and runs scripts/sniff_analyze.py on the pcap+JSON.
#
# Pre-reqs:
...
#
# Usage:
#   bash scripts/run_sniff_experiment.sh                   # 90 s default
#   DURATION_S=120 bash scripts/run_sniff_experiment.sh
#   TDC_FREQ=100 DURATION_S=60 bash scripts/run_sniff_experiment.sh
...
"$PY" scripts/flush_pacing_listener.py \
...
if [ -f "$PCAP" ] && [ -f "$LISTENER_JSON" ]; then
    echo ""
    "$PY" scripts/sniff_analyze.py "$PCAP" \

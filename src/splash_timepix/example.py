...
def main():
    ...
    def data_callback(new_data: np.ndarray) -> None:
        ...
        if len(data_history) % 10 == 0:
            ...
            print(f"📈 Stats - Count: {len(array)}, Mean: {np.mean(array):.2f}, "
    ...
    try:
        # Start the server
        print(f"🚀 Starting server on localhost:8888")
        server.start()
        ...
        while True:
            ...
            if current_time - last_stats_time >= 30:
                ...
                if len(data_array) > 0:
                    print(f"🔢 Recent values: {data_array[-5:] if len(data_array) >= 5 else data_array}")

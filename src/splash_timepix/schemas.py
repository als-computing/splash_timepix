...
class TimePixStop(BaseModel):
    ...
    pixels_discarded_before_trigger: int = Field(
        default=0, 
        description="Total pixels discarded before first TDC trigger"
    )
    pixels_discarded_outside_window: int = Field(
        default=0,
        description="Total pixels discarded outside time window"
    )
...
class TimePixEvent(BaseModel):
    ...
    pixels_discarded_before_trigger: int = Field(
        default=0,
        description="Pixels discarded before first TDC in this flush"
    )
    pixels_discarded_outside_window: int = Field(
        default=0,
        description="Pixels discarded outside time window in this flush"
    )

#!/usr/bin/env python3
"""
Parser for 96-bit TimePix3 live packets (pixel, TDC, control).

Uses NumPy for vectorized :meth:`PacketParser.parse_batch`; :meth:`PacketParser.parse`
handles a single 12-byte packet.

Usage:
    from splash_timepix.parser import PacketParser, PixelPacket, TDCPacket
    parser = PacketParser()
...
class PacketParser:
    ...
    def parse(self, data: bytes) -> Union[PixelPacket, TDCPacket, ControlPacket, None]:
        ...
        Single-packet API; yields one object per 12-byte chunk.
    ...
    def _empty_result(self) -> BatchParseResult:
        ...
        return BatchParseResult(
            ...
            pixel_x=empty_i32,
            pixel_y=empty_i32,
            pixel_timestamp=empty_i64,
            pixel_tot=empty_i32,
            pixel_indices=empty_idx,
            tdc_timestamp=empty_i64,
            tdc_channel=empty_u8,
            tdc_edge=empty_u8,
            tdc_indices=empty_idx,
            control_timestamp=empty_i64,
            control_subtype=empty_u8,
...
if __name__ == "__main__":
    ...
    print(
        f"parse_stream: {count_stream} packets in {t_stream*1000:.1f} ms "
    ...
    print(
        f"parse_batch:  {count_batch} packets in {t_batch*1000:.1f} ms "

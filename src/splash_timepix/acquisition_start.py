...
def spawn_terminal(title: str, command: str, working_dir: Path) -> Optional[int]:
    ...
    try:
        proc = subprocess.Popen(["gnome-terminal", f"--title={title}", "--", "bash", "-c", full_command])
        return proc.pid
...
def wait_for_server_ready(port: int = DEFAULT_HEARTBEAT_PORT, timeout: float = 30.0) -> bool:
    ...
    try:
        ...
        print("WARNING: Heartbeat module not available, using fixed delay")
        time.sleep(5.0)
        return True
...
@app.command()
def main(
    tdc: float = typer.Option(..., "-tdc", "--tdc-frequency", help="TDC trigger frequency in Hz"),
    t: int = typer.Option(
    ...
    preview: bool = typer.Option(False, "--preview", help="Preview mode: stream only, no file writing"),
    ...
    server_command = f"python -m splash_timepix.app " f"--tdc-frequency {tdc} " f"--exit-on-disconnect"
